...
describe('UIRenderer', () => {
    ...
    test('should create all required DOM elements', () => {
        ...
        expect(document.getElementById('gameArea')).not.toBeNull();

        // Check that the overlay was created
        const overlay = document.getElementById('gameOverlay');
        expect(overlay).not.toBeNull();
        expect(document.getElementById('gameOverModal')).not.toBeNull();
    ...
    test('should show game over screen with correct message and scores', () => {
        ...
        expect(playerGoldElement.textContent).toBe(
            `Player Gold: ${playerGold}`,
        );
        expect(computerGoldElement.textContent).toBe(
            `Computer Gold: ${computerGold}`,
        );
        expect(restartButton.textContent).toBe('Play');
    });
    ...
    test('should call restart callback with seed when restart button is clicked', () => {
        ...
        // Seed remains; main entrypoint will set it to the active game seed after restart.
        expect(input.value).toBe(testSeed);
    });
